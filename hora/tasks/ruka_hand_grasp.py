...
class RukaHandGrasp(RukaHandHora):
    ...
    def reset_idx(self, env_ids):
        ...
        if success.any():
            ...
            print("success")
            print('current cache size:', self.saved_grasping_states.shape[0])
            # Clear frames on success so we don't save successful trials
            self.video_frames = []
        else:
            # If resetting but NOT due to success, it's a failure
            # We save the video for the first environment if it's in the resetting batch
    ...
    def post_physics_step(self):
        ...
        # Accumulate frames for the first environment
        ...
        # Keep buffer manageable (e.g., last 250 frames / ~12 seconds)
