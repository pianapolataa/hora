...
class RukaHandGrasp(RukaHandHora):
    ...
    def post_physics_step(self):
        ...
        obj_pos = self.rigid_body_states[0, -1, :3].detach().cpu().numpy().tolist()
        
        # Fingertip indices from your reward function: [Index, Middle, Ring, Pinky, Thumb]
        tip_indices = [6, 9, 13, 17, 20]
        tips_pos = self.rigid_body_states[0, tip_indices, :3].detach().cpu().numpy().tolist()
        
        # 2. Print raw list format for easy plotting
        # Format: [obj_x, obj_y, obj_z, f1_x, f1_y, f1_z, ...]
        print(obj_pos + [item for sublist in tips_pos for item in sublist])

        # Keep your joint angle logic for the .npy file
    ...
    def compute_reward(self, actions):
        ...
        cond1 = (torch.sqrt(((obj_pos - finger_pos) ** 2).sum(-1)) < 0.15).all(-1)
