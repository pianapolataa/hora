...
# --------------------------------------------------------
# In-Hand Object Rotation via Rapid Motor Adaptation
# Modified for 20-DOF Hand Grasp Generation - Headless Video Save
# --------------------------------------------------------
...
class RukaHandGrasp(RukaHandHora):
    def __init__(self, config, sim_device, graphics_device_id, headless):
        # Headless must be True for servers without displays
        super().__init__(config, sim_device=sim_device, graphics_device_id=graphics_device_id, headless=headless)
        ...
        cam_target = gymapi.Vec3(0.01, 0.12, 0.6)
        self.gym.set_camera_location(self.camera_handle, self.envs[0], cam_pos, cam_target)
        # -----------------------------

        # --- DEBUG SAVING LOGIC ---
        self.debug_step_count = 0
        self.debug_joint_angles = [] 
        if not os.path.exists('debug'): os.makedirs('debug')
        # ---------------------------

        self.canonical_pose = [
            0.27, 0.17, -0.16, 0.67, 0.32, 0.32, 0.5, 0.33, 0.33, -0.3, 
            0.67, 0.32, 0.32, -0.12, 0.67, 0.32, 0.32, 0.9, -0.53, 0.2,
        ]
    ...
    def post_physics_step(self):
        ...
        # --- VIDEO FRAME CAPTURE ---
        # Capture frame from env 0 every 2 steps
        if self.debug_step_count % 2 == 0:
            self.gym.render_all_tensors(self.sim)
            self.gym.fetch_results(self.sim, True)
            self.gym.step_graphics(self.sim)
            
            raw_image = self.gym.get_camera_image(self.sim, self.envs[0], self.camera_handle, gymapi.IMAGE_COLOR)
            image_rect = raw_image.reshape(self.camera_props.height, self.camera_props.width, 4)
            self.video_frames.append(image_rect[:, :, :3]) # Keep RGB, drop Alpha
        # ---------------------------
        ...
        # Save video and angles on final step or before exit
        if self.debug_step_count >= self.max_episode_length * 2: # Save after 2 full episodes
            self.save_video_and_exit()
        # ---------------------------

    def save_video_and_exit(self):
        save_path = 'grasp_simulation.mp4'
        print(f"Saving video to {save_path}...")
        imageio.mimsave(save_path, np.array(self.video_frames), fps=30)
        
        angles_path = 'joint_angles.npy'
        np.save(angles_path, np.array(self.debug_joint_angles))
