...
class RukaHandHora(VecTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        pip_idx, dip_idx = [], []
        for i, name in enumerate(dof_names):
            if 'pip' in name.lower(): pip_idx.append(i)
            elif 'dip' in name.lower(): dip_idx.append(i)
            
        if len(pip_idx) == 4 and len(dip_idx) == 4:
            self.pip_indices = to_torch(pip_idx, dtype=torch.long, device=self.device)
            self.dip_indices = to_torch(dip_idx, dtype=torch.long, device=self.device)
            self.non_dip_indices = to_torch([i for i in range(self.num_allegro_hand_dofs) if i not in dip_idx], dtype=torch.long, device=self.device)
        else:
        ...
        obj_pose.p = gymapi.Vec3(-0.17, -0.07, 0.47)
    ...
    def reset_idx(self, env_ids):
        if self.randomize_pd_gains:
            self.p_gain[env_ids] = torch_rand_float(
                self.randomize_p_gain_lower, self.randomize_p_gain_upper, (len(env_ids), self.num_allegro_hand_dofs),
                device=self.device).squeeze(1)
            self.d_gain[env_ids] = torch_rand_float(
                self.randomize_d_gain_lower, self.randomize_d_gain_upper, (len(env_ids), self.num_allegro_hand_dofs),
                device=self.device).squeeze(1)
        ...
        for n_s in range(num_scales):
            ...
            # --- MIMIC JOINT LOGIC: Map saved poses to proper joints if they are 16D ---
            if pos.shape[1] == self.num_allegro_hand_dofs:
                mapped_pos = pos
            else:
                mapped_pos = torch.zeros((len(s_ids), self.num_allegro_hand_dofs), device=self.device, dtype=torch.float)
                mapped_pos[:, self.non_dip_indices] = pos
                mapped_pos[:, self.dip_indices] = mapped_pos[:, self.pip_indices]
            
            self.allegro_hand_dof_pos[s_ids, :] = mapped_pos
            self.allegro_hand_dof_vel[s_ids, :] = 0
            self.prev_targets[s_ids, :self.num_allegro_hand_dofs] = mapped_pos
            self.cur_targets[s_ids, :self.num_allegro_hand_dofs] = mapped_pos
            self.init_pose_buf[s_ids, :] = mapped_pos.clone()
            # --------------------------------------------------------------------------
    ...
    def pre_physics_step(self, actions):
        ...
        # --- MIMIC JOINT LOGIC: Map 16 actions to 20 DOFs ---
        if self.actions.shape[1] == self.num_allegro_hand_dofs:
            action_deltas = self.actions
        else:
            action_deltas = torch.zeros((self.num_envs, self.num_allegro_hand_dofs), device=self.device, dtype=torch.float)
            action_deltas[:, self.non_dip_indices] = self.actions
        # ----------------------------------------------------

        targets = self.prev_targets + 1 / 24 * action_deltas
        self.cur_targets[:] = tensor_clamp(targets, self.allegro_hand_dof_lower_limits, self.allegro_hand_dof_upper_limits)
        
        # --- MIMIC JOINT LOGIC: Force DIP targets to match PIP targets ---
    ...
    def _allocate_task_buffer(self, num_envs):
        ...
        self.proprio_hist_buf = torch.zeros((num_envs, self.prop_hist_len, 40), device=self.device, dtype=torch.float)
